...
#        break
 ...
 #       print(" ----------- Step 2B  -----------")
    ...
    for line in flines:
        ...
        total_bytes = json_data["session"]["total_bytes"]
        ...
        if json_haddix.split('.')[0] == "192":
            ...
            if (str(src_ip) + '.csv') in files_here:
                open(str(src_ip) + '.csv','a').write(str(time) + ',' + str(duration) + "," + str(src_ip)  + "," + str(total_bytes) + ',' + str(entropy) + ',' + str(total_domains) + ',' + str(dict_) + '\n')
            else:
                open(str(src_ip) + '.csv','w').write(str(time) + ',' + str(duration) + "," + str(src_ip)  + "," + str(total_bytes) + ',' + str(entropy) + ',' + str(total_domains) + ',' + str(dict_) + '\n')
